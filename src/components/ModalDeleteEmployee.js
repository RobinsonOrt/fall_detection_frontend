...
const ModalDeleteEmployee = ({ isModalDeleteEmployeeOpen, setIsModalDeleteEmployeeOpen }) => {
    const { deleteEmployee, employee } = useContext(MyEmployeeContext);
    ...
    const handleDelete = async () => {
        ...
        if (response.status === 401) {
            ...
            setError(null);
            setIsModalDeleteEmployeeOpen(false);
        } else {
            setError("Tenemos un inconveniente por favor intente más tarde");
        }
    }
